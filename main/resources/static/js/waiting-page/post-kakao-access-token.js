...
if (authorizationCode) {
    ...
    fetch('http://' + BACKEND_BASE_URL + '/auth/code/kakao', {
        method: 'POST',
        headers: {
    ...
    .then(response => {
        if (!response.ok) {
            document.body.getElementById('step1').innerText='2단계로 진행됩니다. 1단계에 실패했어요.\n피드백 페이지에 상황을 알려주시면,\n빠르게 수정하겠습니다.\n 다시 시도하거나, 비회원으로 진행해주세요.';
            throw new Error('네트워크 응답이 좋지 않습니다.');
        } else {
            document.body.getElementById('step1').innerText='2단계로 진행됩니다. 1단계에 성공했어요.';
    ...
    .then(data => {
        ...
        document.body.getElementById('step2').innerText='2단계에 성공했어요. 이제 퀴즈를 시작할 수 있어요.';
        document.body.getElementById('step3').innerText='Ranking Quiz에 오신 것을 환영합니다.';
        // 로그인 성공 후 처리
        window.location.href = 'http://' + FRONTEND_BASE_URL + '/user-page';
    })
    .catch(error => {
        document.body.getElementById('step2').innerText='2단계에 실패했어요. \n피드백 페이지에 상황을 알려주시면,\n빠르게 수정하겠습니다.\n다시 시도하거나, 비회원으로 진행해주세요.';
    });
} else {
